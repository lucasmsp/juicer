...
from juicer.multi_platform.multi_workflow import MultiWorkflow
from juicer.util.template_util import strip_accents

logging.config.fileConfig('logging_config.ini')
log = logging.getLogger('juicer.multi_platorm.multiplaform_minion')

locales_path = os.path.join(os.path.dirname(__file__), '..', 'i18n', 'locales')


class MultiPlatformMinion(Minion):
    """
    ...
    def __init__(self, redis_conn, workflow_id, app_id, config, lang='en',
        ...
        self._state = {}
        self.transpiler = MultiPlatformTranspiler(config)
        self.config = config
    ...
    def _process_message_nb(self):
        ...
        elif msg_type == MultiPlatformMinion.MSG_PROCESSED:
            self.active_messages -= 1
    ...
    def _perform_execute(self, job_id, workflow, app_configs):
        ...
        start = timer()
        try:
            loader = MultiWorkflow(workflow, self.config)
    ...
    def message_processed(self, msg_type, wid, job_id, workflow):
        msg_processed = {
            'workflow_id': wid,
            'app_id': wid,
            'type': MultiPlatformMinion.MSG_PROCESSED,
            'msg_type': msg_type,
            'job_id': job_id,
            'workflow': workflow
        }
