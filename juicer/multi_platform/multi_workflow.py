...
from juicer.multi_platform.model_calibration import CostModel
...
class MultiWorkflow(object):
    ...
    def __init__(self, workflow_data, config, query_operations=None,
        ...
        # Spark, COMPSs, Scikit-Learn, Keras, Multiplatform
    ...
    def set_platforms(self, lj):

        dataflow = self.cm.gen_dataflow_model_v2(lj)
        jobs = lj.jobs
        print("set_platforms: ", jobs)
        cols_features = list(dataflow.columns)
        cols_features.remove("lemonade_id")
        cols_features.remove("total_seconds")
        cols_features.remove("platform_id")
        rows = dataflow[cols_features].values
        ...
        for i, job_id in enumerate(jobs):
            ...
            for j, task in enumerate(self.workflow['tasks']):
                if task["id"] in tasks:
                    ...
                    print("task_id {} target-platform: {}".format(task["id"], p))

    def optimize_flow(self):
        # Temporario
        default = {"value": {"target": "1"}}
        ...
        for i, task in enumerate(self.workflow['tasks']):
            ...
            if comments in ["1", "4"]:
                ...
                self.workflow['tasks'][i]['forms']["multiplatform"] = default
