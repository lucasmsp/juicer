...
def main(workflow_id, execute_main, params, config, deploy, export_notebook, 
    ...
    slug_to_op_id = dict([(op['slug'], op['id']) for op in ops['data']])
    port_id_to_port = dict([(p['id'], p) for op in ops['data'] for p in op['ports']])
    ...
    try:
        ...
        elif loader.platform['slug'] == 'multi-platform':
            transpiler = MultiPlatformTranspiler(configuration.get_config())
            from juicer.multi_platform.multi_workflow import MultiWorkflow
            loader = MultiWorkflow(resp, config)
        else:
            raise ValueError(
